...
### 一個實例演練：為每個世代新增一列淨營收留存率

分析師在 `SaaS_Model.xlsx`（一份即時的世代營收推算）上，於 Sheets add-on 中輸入「為每個世代新增一列淨營收留存率」。這個請求會走過整條寫入路徑。

**讀取圖，而非網格。** 模型建構器透過 Graph API 拉取活頁簿，重建相依關係而非渲染出的數字。它復原了粒度：`Cohorts` 每個獲客月份一列、第 4 到 15 列（世代 `2025-01` 到 `2025-12`），每個世代的第 0 月 MRR 位於 `Cohorts!C4:C15`，而營收明細是 `Data` 上一份整齊的分類帳，每個（世代、日曆月份）一列、第 2 到 731 列，公開為具名範圍 `led_mrr`（`Data!$C$2:$C$731`）、`led_cohort`（`Data!$A$2:$A$731`）與 `led_month`（`Data!$B$2:$B$731`）。NRR 在這裡定義為最新期間留存的 MRR 除以第 0 月 MRR，所以每一個世代列都需要一個分子，用來加總該世代在 `Latest_Month` 的 MRR。

**生成公式。** 草擬模型針對新欄位 `P` 中的世代第 4 列，提出：

`=SUMIFS(Data!$C$2:$C$730, Data!$A$2:$A$730, $B4, Data!$B$2:$B$730, Latest_Month) / $C4`

這個臭蟲是一個柵欄樁差一：它把三個範圍明確打成 `...$2:$730`，比結束於第 731 列的 `led_mrr` 區塊少了一列。因為這些範圍是絕對參照，這個短缺總是丟掉最末一列分類帳列，那一列屬於世代 `2025-12`（第 15 列），帶著它最新月份 MRR 的 $8,400。

**沙箱重新計算核對。** 閘門通過了便宜的確定性檢查（它解析成功、參照解析得出），並用 `formulas` 引擎在一份沙箱副本中重新計算整個新欄位。世代 `2025-12` 回傳 $117,600 / $120,000 = 0.980，一次收縮。接著核對以第二種、獨立的方式重算同一個量，一次會計交叉合計：它加總十二個各世代分子，這必須與一個獨立地在完整 `led_mrr` 範圍上取得的最新月份總計相符，卻得到 $1,412,100，對上 $1,420,500 的總計。差距正好是 $8,400，比核對 epsilon（相對 `1e-9`、絕對 `$0.005`）大了好幾個數量級。同時，參照與範圍檢查標記出 `Data!$C$2:$C$730` 比連續的 `led_mrr` 區塊少停了一列。兩個訊號都收斂到同一個柵欄樁差一上。這正是 [Reinhart-Rogoff](https://en.wikipedia.org/wiki/Growth_in_a_Time_of_Debt) 的形態：一個聚合範圍默默丟掉了邊界列並翻轉了一項頭條，在這裡是 `2025-12` 從 105 percent 擴張變成 98 percent 收縮。

**修復，然後一份受追蹤的 diff。** 這個攔阻導向修復；因為一次確定性檢查把成因定位到一個範圍邊界，Opus 4.8 把三個範圍改寫成受維護的具名範圍（`led_mrr`、`led_cohort`、`led_month`），好讓公式能自我延伸。重新計算為 `2025-12` 回傳 $126,000 / $120,000 = 1.050，交叉合計相符到分，整個欄位也核對通過。直到此刻才渲染出一份 diff（新欄位 `P`、逐列的舊對新並附重新計算的預覽）。分析師核准；寫入透過 API 回寫，相依封閉集重算後沒有新的 `#REF!`，這次變更落進了可還原的日誌。沒有任何東西被自動套用，也沒有任何既有公式被觸碰。

### 在稽核模式中的同一個模型

把代理以唯讀方式指向同一份 `SaaS_Model.xlsx`，它就會獵捕那些經典的臭蟲類別，而不是建構。浮現出兩項發現：

- **公式裡寫死的數字。** `Cohorts!H9` 內容為 `=G9*1.08`，其中 `1.08` 是一個經流失調整的成長假設，被當成字面值打進去，而不是對 `Assumptions` 工作表 `Gross_Churn` 驅動格的一個參照，所以當分析師調動這個假設時它永遠不會動。稽核器標記出這個數值字面值（在真常數的允許清單之外，例如 `12` 個月），並提議 `=G9*(1-Gross_Churn)`。這是 JPMorgan 那一類的運算子／填充值。
- **跨一列不一致的公式。** `Cohorts!F4:F15`（第 3 月留存的 MRR）在每一個儲存格都是一致的 `=SUMIFS(...)`，唯獨 `F11` 例外，它在幾個月前被手動改成 `=SUMIFS(...)+500`，一次早已被遺忘的手動校正。[ExceLint](https://arxiv.org/abs/1901.11100) 風格的一致性偵測會把 `F11` 標記為一個一致範圍中唯一的異常值，並依波及範圍（有多少下游儲存格依賴它）排序。代理什麼都不寫；它產出排序後的發現。

這個對比正是重點：同一份相依圖讀取，既驅動一次有把關的寫入，也驅動一次零寫入的稽核，而稽核所鎖定的錯誤類別（差一範圍、寫死的填充值、不一致的列）正是 [EuSpRIG](https://eusprig.org/research-info/horror-stories/) 記錄所構築起來的那些。

### 公式變更紀錄

每一次提議的編輯都被產出為一份經 schema 驗證的變更紀錄，而非自由散文，好讓閘門與稽核日誌在同一個結構化物件上推理。這是那個已修復的 `2025-12` 儲存格的紀錄：
...
閘門只信任結構化的 `validation` 區塊；一份 `recalc_reconciled` 為 false 或帶有任何 `downstream_errors` 的紀錄，永遠無法抵達核准佇列，而對一次寫入來說 `requires_approval` 永遠為 true。
...
一條生成的公式是一個假設，而非一個答案，而且它要先通過一道由程式碼構成的閘門，才輪到通過那道憑感覺的閘門。這是 [text-to-SQL correctness gate](39-conversational-analytics-text-to-sql.md) 的直接類比，它先跑確定性檢查，因為那些檢查是免費的：公式被解析成一個 AST、每一個儲存格參照與範圍都對照真實網格加以驗證（`Revenue` 是否解析得出、範圍是否涵蓋了預期的列），型別與單位也都經過合理性檢查，好讓一個成長率永遠不會被 `SUM`、一個單位價格永遠不會被加進一個總計。只有在便宜又確定的檢查通過之後，昂貴的機率性檢查才會執行。這就是 [guardrails](../13-reliability-and-safety/01-guardrails.md) 的紀律：絕不花一次 LLM 呼叫，去捕捉一個解析器免費就能捕捉的東西。為這道閘門評分的黃金測試集評估，是一套 [SpreadsheetBench](https://arxiv.org/abs/2406.14991) 風格的「任務、之前活頁簿、之後活頁簿」三元組，所以「正確」意味著重新計算的結果相符，而不是公式讀起來看似合理。這道閘門是一個固定的有序序列，而每一項檢查都對應到寫入、攔阻或升級：

| 驗證檢查（依序） | 通過 | 失敗 |
|---|---|---|
| 解析為一個有效的公式 AST | 下一項檢查 | 攔阻、自動修復 |
| 參照與具名範圍解析成功 | 下一項檢查 | 攔阻、自動修復 |
| 範圍涵蓋完整的連續區塊（無柵欄樁差一） | 下一項檢查 | 攔阻、自動修復（差一／Reinhart-Rogoff 類） |
| 型別與單位一致（比率不被加總、貨幣不除以計數） | 下一項檢查 | 攔阻、自動修復 |
| 沙箱重新計算未引發 `#REF!`、`#DIV/0!` 或循環參照 | 下一項檢查 | 攔阻、自動修復 |
| 在 epsilon 內與一次獨立推導核對相符 | 寫入（渲染 diff） | 若一項檢查定位出成因則攔阻以修復，否則升級給分析師 |
| 分析師核准所渲染的 diff | 回寫加上重新計算封閉集 | 丟棄並記錄 |

「攔阻」會跑自動修復迴圈（重新草擬，或把修復本身升級給 Opus 4.8）；「升級」則是把這個不一致交給真人，而不是去猜它。這個實例演練在世代 `2025-12` 上未通過核對那一列，但因為一同觸發的範圍檢查把成因定位到一個單列柵欄樁差一，它會攔阻以進行自動修復，而非升級，這一切都在寫入任何一個儲存格之前發生。

### 3. 引擎才是計算機，模型絕不是

最誘人的失敗，是 LLM 在腦子裡做算術。它絕對不可以，一次都不行。代理所產生或所依賴的每一個值，都在一個真正的試算表引擎（`formulas`、[PyCel](https://github.com/dgorissen/pycel) 或無頭 LibreOffice 的重新計算）中、於一份沙箱副本上執行，其結果會在顯示之前完成核對。這正是試算表特有的狀態會咬人的地方：`#REF!`、`#DIV/0!` 與 `#N/A` 必須被偵測出來並誠實地傳播，而不是被粉飾過去；循環參照必須被找出來，並以反覆運算加以解決，或予以拒絕；而浮點漂移意味著「核對」是一種 epsilon 比較，而非 `==`。執行而非信任，跟執行 SQL 而非相信模型預測的結果，是同一種直覺，但這裡的暴露面更大，因為單單一次編輯，就可能默默改動五十列以外的一個儲存格。核對刻意是一次雙推導的一致性檢查，而非單一次重算：沙箱執行生成的公式，一次獨立推導用不同的方式重算同一個目標（一次對總計的交叉合計、一次在來源分類帳上的 groupby，或一個封閉形式恆等式），而兩者必須在一個 epsilon（相對 `1e-9`、絕對 `$0.005`）內相符，因為浮點漂移讓 `==` 成為一個臭蟲。在這個實例演練中，那次交叉合計正是揭露那 $8,400 短缺的東西，而單靠重新計算原本會把它回報成一個乾淨、自信、錯誤的 98 percent。

### 4. 稽核是一等公民的唯讀模式

鑑於大多數營運用試算表本來就含有錯誤（[Panko / EuSpRIG](https://arxiv.org/abs/0802.3457)），稽核不是一個附帶功能，它是產品的一半，而且它什麼都不寫。稽核器獵捕那些經典、昂貴的臭蟲：埋在公式裡寫死的數字（`=A1*1.2` 裡的 `1.2` 本該是一個驅動儲存格）、跨一列或一欄不一致的公式（其中某個儲存格被手動改過）、損壞的外部連結，以及正負號錯誤。秉持 [ExceLint](https://arxiv.org/abs/1901.11100) 精神、以結構化、基於一致性的偵測，會以確定性方式找出範圍內的異常公式，再由一個 LLM 層來解釋並排定優先順序。這些正是那些著名災難背後確切的形態：漏掉五個國家的 [Reinhart-Rogoff](https://en.wikipedia.org/wiki/Growth_in_a_Time_of_Debt) 範圍，以及 JPMorgan 的 [sum-instead-of-average](https://en.wikipedia.org/wiki/2012_JPMorgan_Chase_trading_loss)。發現會依波及範圍（模型有多少部分依賴那個可疑儲存格）排序，好讓分析師先看到那個承載關鍵的錯誤。這個實例演練中的稽核模式跑法正是這個行為：在同一份 `SaaS_Model.xlsx` 上，它標記出 `H9` 寫死的 `1.08`（一個本該參照 `Gross_Churn` 的驅動值）以及 `F11` 在一個原本一致的列中那個唯一的 `+500` 異常值，什麼都不寫。這些並非假設：每個儲存格 1 到 5 percent 的錯誤率（[Panko / EuSpRIG](https://arxiv.org/abs/0802.3457)）意味著一個 2,000 條公式的模型帶著數十個潛伏的臭蟲，所以稽核召回率是一項頭條 SLO，而不是一個可有可無的裝飾。
...
核對是那道承載關鍵的檢查，所以值得單獨來看。代理絕不信任單一次重算；它以兩種獨立方式推導同一個目標量，並要求它們在一個浮點 epsilon 內相符。單獨對一條錯誤公式做一次重新計算，會自信地回傳一個錯誤的數字（上面那個 98 percent）；唯有一次不一致的獨立推導才能揭露它。當它們不一致時，一次定位出成因的確定性檢查（一個柵欄樁範圍、一個循環）會把一次棄答轉化為一次機械式的自動修復；一次無法解釋的不一致則會升級給分析師，而不是去猜。

```mermaid
flowchart TD
    Q[目標量 各世代 NRR] --> A[推導 A 在沙箱中重新計算生成的公式]
    Q --> B[推導 B 對總計的獨立交叉合計]
    A --> CMP{A 與 B 在 epsilon 內相符？}
    B --> CMP
    CMP -->|yes| PASS[核對相符 渲染 diff 供核准]
    CMP -->|no| LOC{確定性檢查是否定位出成因？}
    LOC -->|yes fencepost or cycle| REPAIR[攔阻 透過 Opus 4.8 自動修復]
    LOC -->|no| ESC[升級給分析師 棄答]
    REPAIR --> A
```
