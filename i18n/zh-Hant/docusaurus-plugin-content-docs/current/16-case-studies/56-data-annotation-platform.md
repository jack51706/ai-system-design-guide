...
### 實例演練：單一 10,000 筆的內容分類批次

單獨一批就能看出這套設計的價值何在。一個下游團隊需要把 10,000 則社群論壇貼文，依一套 6 類的主題分類法（product-feedback、billing、technical-support、feature-request、spam、off-topic）標註，以訓練一個意圖路由器。這個任務夠客觀，足以自動接受容易的多數，而這正是 LLM 預標所適用的情境。

**預標與路由。** Haiku 4.5 為全部 10,000 則貼文預標，每則都附一個校準過的信心（token logprobs 與五個自我一致性樣本混合而成），同時 Snorkel 標註函式（billing 關鍵字 regex、URL 密度的 spam 啟發式規則、一條長度與大寫規則）平行投票，再由標籤模型融合它們。路由器自動接受了那 6,400 則信心超過 0.92、標籤模型也同意、且類別非敏感的貼文。其餘 3,600 則交給人工：約 1,900 則低信心（低於 0.75）、約 1,400 則 LLM 與標籤模型分歧（query-by-committee），以及約 300 則被標記為可能騷擾、採盲標優先且永不可自動接受的貼文。為 3,600 筆而非 10,000 筆項目付人工錢，正是整個經濟論據所在。

**即時量測品質。**一份 500 則人工審查貼文的雙標切片會交給第二位獨立標註者，整體 Cohen's kappa 為 0.81（在 Landis 與 Koch 的分級上屬於幾近完美），高於 0.75 的出貨門檻。但 feature-request 對 product-feedback 這一對，單獨來看只拿到 0.53，這標出了一處指引的含糊（標註者並不是偷懶，而是那條邊界界定不清），於是在出貨前先把指引釐清並重新標註那一份切片。

**用蜜罐抓出自動化偏誤。**黃金與蜜罐項目以人工佇列的 8 percent 植入（約 290 個探針）。其中一個蜜罐是一則明顯屬於 billing-refund 的貼文，卻附上一個故意設錯、在 model_conf 0.93 下標成 spam 的預標。標註者 A22 接受了 spam，也就是對模型蓋了橡皮圖章。蜜罐抓到了它：A22 本週已經漏掉 11 個蜜罐中的 3 個，且在顯示的預標上只有 2 percent 的編輯率，對照群體中位數的 14 percent，這是自動化偏誤的雙重特徵。A22 被自動停權，其近期標籤排入重新裁決佇列。標註者 A08 看到同一個蜜罐，把它改成 billing 並通過。

**把循環性陷阱講具體。**項目 7731 是一則使用了新俚語詞的貼文，被以 0.94 預標成 off-topic 並自動接受。接受路徑上沒有任何一個環節見過人工。抓到它的，是對自動接受資料流的常設黃金稽核：一份新鮮的 2 percent 人工標註樣本顯示，那個俚語叢集的稽核精確率掉到了 91 percent，低於 98 percent 的門檻。這個叢集被撤出自動接受、路由給人工，且其近期的自動接受被重新裁決。這正是整個平台存在的目的所要防範的陷阱：少了那道稽核，一個自信而錯誤的預標就會無聲地變成訓練下一個模型的標準答案，而模型接著會以更高的信心學到同一個盲點。模型永遠不被允許為自己的標籤背書。

**瞄準下一批。**在這一批上訓練之後，刷新過的模型會跑過大約 2M 筆未標註的積壓，主動學習選出 10,000 筆最高不確定性的項目（不確定性抽樣加上 query-by-committee），而那個新的俚語叢集在這次選取中佔了主導，於是下一批把人力正好花在模型最弱之處，而非齊頭式地花。不同於[合成資料生成管線](37-synthetic-data-generation.md)製造全新的輸入，這裡每一筆標籤都可回溯到一筆真實項目，並在有爭議的那些項目上回溯到一位具名的存案人工。

### 一筆標籤記錄範例

每一筆項目都帶著它完整的路由與溯源，好讓一張資料集卡片與一次稽核能精確重建這個標籤是怎麼產生的。上面那一批裡的三筆記錄道出了整個故事：一筆事後被稽核抓到的自動接受、一個被審查者蓋了橡皮圖章的蜜罐，以及一次真正的人工修正。
...
核心的經濟操作。一個便宜的 LLM 為全部 10M 筆項目預標；路由器自動接受有信心且彼此一致的多數，只把不確定且有爭議的少數送給人工。自動接受門檻直接在成本與品質之間做取捨，所以要以經驗來設定它：挑出那個「經稽核、對照黃金的自動接受準確率仍維持在資料集品質門檻（比方說 98 percent）之上」的最高門檻，並把低於它的一切都路由出去。實務上 55 到 75 percent 的自動接受是常態，而這正是「為 10M 筆項目付人工錢」和「只為 3M 筆付錢」之間的差別。在上面那個實例演練裡，這條規則自動接受了 10,000 則貼文中的 6,400 則，把 3,600 則送給人工。這套路由是一張可核對的表，不是憑感覺：

| 模型信心 | 標籤模型／弱監督的一致性 | 類別 | 路由 |
|---|---|---|---|
| 超過 0.92 | 一致 | 客觀、非敏感 | 自動接受（搭配黃金稽核抽樣） |
| 超過 0.92 | 分歧 | 任何 | 人工審查（query-by-committee） |
| 0.75 到 0.92 | 一致 | 客觀 | 人工審查（單輪） |
| 0.75 到 0.92 | 分歧 | 任何 | 人工審查，接著共識 |
| 低於 0.75 | 任何 | 任何 | 人工審查（盲標優先） |
| 任何 | 任何 | 敏感或主觀 | 盲標優先人工，永不自動接受 |
| 審查後標註者分歧 | 不適用 | 任何 | 先共識，再裁決 |

這是教科書式的 human-in-the-loop 路由；參見 [Human-in-the-Loop Patterns](../07-agentic-systems/08-human-in-the-loop-patterns.md)。
...
人工標籤是稀缺資源，所以要把它們花在最能改變下游模型的地方，而不是齊頭式地花。不確定性抽樣（標註模型最沒把握的項目）與委員會查詢（query-by-committee，標註 LLM 與標籤模型分歧之處）是經典的槓桿（[Settles, 2009](https://burrsettles.com/pub/settles.activelearning.pdf)）。這道飛輪是：標註一批、訓練、把新模型跑過未標註資料、找出困難與新近變得不確定的案例，然後把那些下一輪送給人工。這和評估堆疊用來尋找失效切片的，是同一個不確定性訊號；參見 [LLM Evaluation](../14-evaluation-and-observability/01-llm-evaluation.md)。做得好的話，主動學習只需要隨機抽樣所需標籤的一小部分，就能達到目標準確率。在這個實例演練裡，刷新過的模型跑過了大約 2M 筆未標註的積壓，主動學習為下一輪選出 10,000 筆最高不確定性的項目，由新發現的俚語叢集主導，於是下一批把人力集中在模型剛剛被抓到失敗之處。
...
自動接受是唯一一個標籤不經人工就出貨的地方，所以它享有安全關鍵等級的待遇：一道常設稽核，用來阻止模型為自己的標準答案背書。對自動接受資料流的一份新鮮、完全由人工標註的樣本會被持續評分，而只要任何類別或叢集上的稽核精確率掉到品質門檻以下，那一份切片就會被拉回給人工，且其近期的自動接受會被重新裁決。這正是打破循環性陷阱（決策 3）的具體機制，也正是它在實例演練中抓到了項目 7731。

```mermaid
flowchart TD
    AA[自動接受資料流] --> SAMP[新鮮人工黃金樣本 2 percent]
    SAMP --> P{稽核精確率達到或高於 98 percent?}
    P -->|是| KEEP[此類別維持自動接受]
    P -->|否| PULL[把類別或叢集撤出自動接受]
    PULL --> HUMAN[把切片路由給人工審查]
    PULL --> AL[主動學習鎖定該切片]
    HUMAN --> RELABEL[重新裁決近期的自動接受]
    KEEP --> GT[(標準答案標籤儲存庫)]
    RELABEL --> GT
    AL -.下一批.-> HUMAN
    GT -.訓練.-> NEXT[下一個模型]
