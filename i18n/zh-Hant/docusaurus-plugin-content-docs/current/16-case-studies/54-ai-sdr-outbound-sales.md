...
### 一個實戰範例：一位 Series B 公司的工程副總裁

跟著一位潛在客戶從名單走到回覆。賣方是一家內部開發者平台廠商，而潛在客戶是 Cirrus Robotics 的工程副總裁 Dana Okafor。

**進件與法遵。** Dana 從「post-Series-B scaling」活動名單進入，綁定到一位業務與美國（US）區域。法遵閘門會檢查全域抑制清單（不在其中）、先前的退訂紀錄（沒有），以及一個美國商業聯絡人的合法依據（CAN-SPAM 之下的選擇退出依據），所以 Dana 符合資格；假如她是一位歐盟聯絡人，閘門則會改為要求一份在 GDPR 與 ePrivacy 之下有文件佐證的正當利益評估，並在更嚴格的會員國取得事先同意。同一份名單上的第二位 Cirrus 聯絡人，也就是那位 CTO，四個月前從一場網路研討會活動退訂了，所以即使這個帳戶如今正熱，閘門仍會橫跨每一個信箱與網域，把那個地址直接抑制掉。一個帳戶，兩種閘門結果：符合資格與受抑制。

**以依據為本的研究。** 研究層檢索到了那個觸發事件：Cirrus 在 2026-06-18 宣布了一輪 4,500 萬美元的 Series B，並由兩個獨立來源（募資公告與 SEC Form D 申報）相互佐證，兩者都附出處快取起來。它也拉取了 Dana 的職務與年資（工程副總裁，14 個月）以及一個技術棧訊號（一則點名 monorepo 與自架 CI 的徵才貼文）。每一項事實都帶有一個來源，而一個誘人的「剛遷移到 Kubernetes」細節回傳時卻沒有引用來源，所以提煉器在生成之前就把它丟棄了。

**生成與一次被逮到的誇大不實。** Sonnet 4.7 草擬了一封以 Series B 與 CI 訊號為依據的首次觸及，帶著一個低摩擦的訴求（一通 20 分鐘的通話）。為了增添說服力，這份草稿憑空捏造了一個佐證點：「我們幫助一家 Series-B 的機器人公司削減了 40 percent 的基礎設施支出。」輸出防護機制以兩個各自獨立的理由駁回了它：其一，這個客戶案例並不在已核准的具名參照允許清單內（它捏造了一個公司無法背書的成功故事）；其二，那個 40 percent 的數字並非一項已公開發表、經核准的數據。防護機制保留了這份草稿、剝除了那項捏造的聲明，而產生器只用一個已核准的價值主張重寫（「各團隊把 CI 等待時間的中位數從 9 分鐘降到 4 分鐘」）。草稿一被保留、草稿二通過，而這就是「寄送還是保留」這個決策活生生的樣子。

**送達率閘門。** 控制平面挑選了一個位於某網域、處於預熱第 34 天、當天 40 封寄送量還剩 18 封的已預熱信箱，確認 SPF、DKIM 與 DMARC 對齊，讀取到 Google Postmaster 信譽為 High、網域投訴率為 0.04 percent（遠低於 0.3 percent 這條線），並確認該地址通過了驗證且不是一個垃圾郵件陷阱。全部亮綠燈，所以這次寄送被安排在一個貼近真人的時間。

**回覆與交接。** 三天後 Dana 回覆：「時機正好，我們正要把平台團隊擴編一倍，給我幾個時段吧。」Haiku 4.5 分類器把它標記為有意願、序列引擎停止了每一次剩餘的觸及，而這個討論串連同完整的研究與討論串脈絡被交給了一位真人客戶主管。AI 可以提議行事曆時段，但它絕不協商條款，因為買方有意願的那一刻，正是真人成交者接手的時候。

重點在於：模型草擬了文案，但真正決定什麼東西實際送出去的，是經查證的出處（被引用的 Series B、允許清單內的價值主張）與確定性的閘門（抑制、合法依據、信譽），而擁有這筆交易的是一個真人，不是這個代理。

### 對外觸及紀錄

這套系統會為每一次觸及送出一筆通過綱要驗證（schema-validated）的紀錄，好讓法遵、送達率與依據性成為閘門能讀取的可檢查欄位，而不是只能信任的一段文字。以下就是上述那次寄送的紀錄。
...
這正是把真正的設計與示範區分開來的那個不顯眼的系統性問題。信箱服務商會為寄件網域與 IP 的信譽評分，而那份信譽就是這條管道的命脈：一旦失去它，一切（包括業務親手寫的郵件）都會掉進垃圾郵件。所以控制平面掌管四件事。驗證：每個網域都發布 SPF、DKIM 與 DMARC，而寄送必須對齊，因為 Google 與 Yahoo 如今會拒收未通過驗證的大量郵件，或把它們歸進垃圾郵件（[RFC 7489](https://datatracker.ietf.org/doc/html/rfc7489)）。預熱與寄送量上限：新網域緩慢爬升，而每個信箱每天的陌生寄送量被限制在 30 到 50 封上下，這正是為什麼每月 500,000 次觸及會逼出一個橫跨數十個網域的數百信箱池，並加以輪換，好讓沒有任何單一資產暴衝。信譽監控：投訴率會對照 Google Postmaster 監看，並穩穩維持在 0.3 percent 門檻之下，而一旦攀升就會自動限流或暫停出問題的那個網域。避開垃圾郵件陷阱：絕不購買名單、驗證每一個地址，並淘汰陳舊的聯絡人，因為命中原生或回收的 [spam traps](https://www.spamhaus.org/faq/section/Spamtraps) 會讓信譽迅速崩跌。預熱是一份時程表，而不是一個開關：一個全新的網域從每天約 20 封開始，並在 4 到 8 週間逐步爬升，之後才會承載每天滿 40 封的陌生寄送量，而 [Google Postmaster](https://support.google.com/mail/answer/9981691) 信譽會被讀成一個等級帶（High、Medium、Low、Bad），用來閘控一個網域可以推得多用力。在這個實戰範例中，被選中的信箱處於預熱第 34 天、信譽為 High、投訴率為 0.04 percent，這正是它得以通過的原因，而一個讀數為 Low 或暴衝越過 0.3 percent 的網域，則會在它能寄送之前就被限流或暫停。有量無質不會讓對外開發規模化，只會摧毀它。

### 2. 以依據為本的個人化：研究潛在客戶，每項事實都引用來源

個人化唯有在為真時才有幫助。這套系統會檢索關於帳戶與人物的事實（募資新聞、產品發布、潛在客戶的職務與年資、從徵才貼文或 BuiltWith 之類訊號取得的技術棧），並把訊息以那些附帶出處、檢索得來的事實為依據，這與 [RAG fundamentals](../06-retrieval-systems/01-rag-fundamentals.md) 是同一套紀律。一句從未發生、幻覺出來的「恭喜完成 C 輪募資」比通用範本還糟，因為它證明了寄件者是個機器人、根本沒下功夫。所以產生器只能引用附帶來源回傳的事實、提煉器會丟棄沒有引用來源的事實，而防護機制（決策 4）則會重新查核草稿中每一項具體聲明都能追溯到一個檢索到的來源。當研究資料稀薄時，訊息會優雅地降級到一個角色層級的相關切角，而不是憑空捏造一個細節。帳戶研究會被快取並共享，所以要做到精準所付出的成本，是每個帳戶付一次，而非每封郵件付一次。這道門檻是出處，而對於一個當頭的觸發事件，門檻則是相互佐證：在這個實戰範例中，那輪 Series B 之所以可用，唯一的原因是它來自兩個獨立來源（募資公告與 SEC Form D），而一個沒有引用來源的「剛遷移到 Kubernetes」臆測，則在它能抵達草稿之前就被丟棄了。
...
一封陌生開發郵件是品牌的一則公開聲明，所以輸出防護機制很嚴格。一份聲明允許清單會列舉系統獲准主張的內容（已核准的價值主張、真實且具名的客戶案例、已公開發表的數據），而任何在清單之外的聲明都會被封鎖：不得有捏造的案例研究、不得有杜撰的指標（毫無根據地說「削減成本 40 percent」）、不得有假造的急迫感（「只剩 2 個名額」），也不得假冒某位真實具名的同事或虛構一段既有關係。一個依據性驗證器會把個人化聲明繫回檢索到的來源，而一個輕量分類器則會標記操縱性或不合規的措辭。這正是 [Guardrails](../13-reliability-and-safety/01-guardrails.md) 模式，只是瞄準的是品牌與法律風險，而非有害內容：這裡的失效模式是一個流暢、篤定卻虛假的句子，所以防護機制會把草稿當成一項待查證的聲明，而不是一段可以信任的文字。這個實戰範例展示了該機制的實際觸發：一份憑空捏造佐證點的草稿（「我們幫助一家 Series-B 的機器人公司削減了 40 percent 的基礎設施支出」）以兩個各自獨立的理由被駁回，其一是這個客戶案例並不在已核准的具名參照允許清單內，其二是那個 40 percent 的數字並非一項已公開發表的數據，而這份草稿會一直被保留，直到它被重寫為一個已核准的價值主張為止。給出兩個各自獨立的駁回理由正是本意，因為其中任一個單獨都應該要能擋下這次寄送。
...
互動與信譽之間的張力就是整場賽局，而選錯指標就會輸掉它。原始寄送量與開信率兩者都會獎勵垃圾郵件大砲式的行為，而且在 [Apple Mail Privacy Protection](https://www.apple.com/newsroom/2021/06/apple-advances-its-privacy-leadership-with-ios-15-ipados-15-macos-monterey-and-watchos-8/) 預先抓取像素之後，開信率本來就不可靠。所以北極星指標是正向回覆率與預約到的會議，並在其上以垃圾郵件投訴率與退訂率作為額外的硬性防護機制。這套系統會為了更少、更好、更有依據且能贏得回覆的觸及而最佳化，並把攀升的投訴率當成一個凌駕任何衝量目標的停止訊號。一場把寄送量增為三倍、正向回覆卻持平而投訴上升的活動，是在失敗，不是在規模化，而儀表板正是為了讓這一點一目了然而打造的。真正重要的單位是每一千次有依據的觸及所換得的正向回覆，而非原始寄送數，如此一來，一個團隊就無法單靠多寄一些就製造出成功的假象，而那恰恰是錯誤指標會獎勵的行為。
...
每一次寄送都要通過同一道有序的閘門，而結果有三種，不是兩種：可修正的問題會保留該次觸及、永久性的問題會抑制該聯絡人，而只有完全乾淨的觸及才會寄送。

| 寄送當下的條件 | 結果 |
|---|---|
| 在全域抑制清單上或先前已退訂 | 抑制（永不聯絡，橫跨所有信箱） |
| 該區域沒有合法依據，或屬於消費者或受禁運的聯絡人 | 抑制 |
| 某項個人化聲明無法追溯到一個引用來源 | 保留（精簡為角色層級，然後重新查證） |
| 某項聲明不在允許清單內，或使用了假造的急迫感或假冒身分 | 保留（改寫或轉給真人審查） |
| 信箱未預熱或已達每日上限 | 保留（排入下一個時段佇列） |
| 網域信譽下滑或投訴率超過 0.3 percent | 保留（限流或暫停該網域） |
| 所有檢查通過 | 以貼近真人的速率寄送 |
...
```mermaid
sequenceDiagram
    participant L as 潛在客戶與不受信任的回覆
    participant C as 法遵閘門
    participant R as 研究與提煉器 Haiku 4.5
    participant G as 產生器 Sonnet 4.7 與防護機制
    participant D as 送達率控制平面
    participant H as 真人業務

    C->>C: 檢查抑制、退訂、合法依據
    Note over C: 受抑制或無依據就到此為止
    C->>R: 符合資格的潛在客戶
    R->>G: 只給有引用來源的事實，未引用的丟棄
    Note over G: 防護機制駁回不在允許清單內的客戶聲明
    G->>G: 改寫為一個已核准的價值主張
    G->>D: 以依據為本、聲明已查證的草稿
    D->>D: 檢查預熱、餘量、SPF DKIM DMARC、信譽
    alt 所有閘門通過
        D-->>L: 以貼近真人的速率寄送
    else 信譽下滑或無餘量
        D->>D: 限流、暫停或排入佇列
