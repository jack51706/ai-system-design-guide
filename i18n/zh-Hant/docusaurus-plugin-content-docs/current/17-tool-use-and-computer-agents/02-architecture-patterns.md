...
```mermaid
flowchart TD
    A["使用者訊息"] --> B["LLM 推理<br/>（我需要查詢這筆訂單）"]
    B --> C["工具選擇<br/>tool: search_db<br/>args: {id: 42}"]
    C --> D["工具執行器（框架）<br/>驗證參數<br/>呼叫函式<br/>回傳結果"]
    D --> E["結果注入情境<br/>{status: shipped, tracking: 1Z...}"]
    E --> F["LLM 產生最終回應"]
```
...
```mermaid
flowchart TD
    G["任務目標（自然語言指令）<br/>用上週的收據填寫費用報銷表單"] --> Loop
    subgraph Loop["VISION-ACTION 迴圈（重複直到完成）"]
        direction LR
        O["OBSERVE（觀察）<br/>截圖（base64）"] --> R["REASON（推理）<br/>分析截圖<br/>+ 目標 + 歷史 + 先前動作"]
        R --> A["ACT（執行）<br/>發出動作<br/>{type: click, x: 450, y: 320}"]
        A --> O
    end
    Loop --> Sandbox
    subgraph Sandbox["沙箱化環境（VM / Docker + VNC）"]
        direction LR
        D1["桌面 (Xfce)"]
        D2["瀏覽器 (Chrome)"]
        D3["應用程式（任意）"]
...
```mermaid
flowchart TD
    U["使用者（自然語言）：分析這個 CSV 並繪製前 10 名產品"] --> A["LLM 產生程式碼<br/>Python/Bash/JS"]
    A --> B["權限關卡<br/>執行這段程式碼？[y/N]<br/>（自動核准、永遠詢問或基於規則）"]
    B --> C["程式碼執行器<br/>執行，擷取 stdout/stderr/回傳值"]
    C --> D["輸出觀察器<br/>有錯誤？回饋給 LLM 修正。<br/>成功？呈現給使用者。"]
    D --> E["LLM 決策<br/>完成？回傳結果。<br/>還需要？產生下一個程式碼區塊。"]
    E -->|"迴圈"| A
```
...
```mermaid
flowchart TD
    U["使用者請求"] --> O["編排器 ORCHESTRATOR（Frontier 模型：Claude Opus、GPT-4o）<br/>分析任務、選擇代理、路由並等待"]
...
| 做法 | 步驟 | 成本 |
|----------|------|------|
| 傳統 | Frontier Model 處理所有步驟 | $1.00/task |
| Plan-and-Execute | Frontier Model 規劃（1 次呼叫） | $0.05 |
| Plan-and-Execute | Small Model 執行步驟 1-3 | $0.03 |
| Plan-and-Execute | Frontier Model 彙整（1 次呼叫） | $0.05 |
| Plan-and-Execute | **合計** | **$0.13/task（節省約 87%）** |
...
| 面向 | 非沙箱化（主機存取） | 沙箱化（隔離） |
|--------|---------------------------|----------------------|
| 執行方式 | LLM 輸出直接在主機 OS 上執行 | LLM 輸出在 Docker/VM/E2B 內部執行 |
| 風險概況 | 風險：`rm -rf /`、資料外洩 | 隔離的檔案系統、網路、行程 |
| 使用者 | OpenClaw、Open Interpreter、Claude Code（預設） | OpenHands、OpenAI Codex、Jules、Cursor Background Agents |
...
```mermaid
flowchart TD
    A["LLM 產生程式碼/工具呼叫"] --> B["執行"]
    B --> C{"成功？"}
    C -->|"是"| D["回傳結果"]
    C -->|"否"| E["把錯誤 + stderr 回饋給 LLM"]
    E --> F["LLM 產生修正<br/>（最多 5 次修正以防止無窮迴圈）"]
    F --> B
```
...
flowchart LR
    ...
    A2["Agent 2"] --> GW
    A3["Agent 3"] --> GW
    GW["MCP Gateway<br/>（驗證、速率限制、稽核、路由）"] --> G["GitHub MCP"]
    GW --> P["Postgres MCP"]
    GW --> S["Slack MCP"]
```
...
flowchart TD
    Q1{"目標系統有 API 嗎？"}
    Q1 -->|"有"| P1["模式 1（工具呼叫）<br/>包裝成 MCP server。最快、最可靠。"]
    Q1 -->|"沒有"| Q2{"任務需要 GUI 互動嗎？"}
    Q2 -->|"需要"| P2["模式 2（基於視覺）<br/>在 VM 中沙箱化。接受延遲。"]
    Q2 -->|"不需要"| Q3{"任務主要是程式碼/資料工作嗎？"}
    Q3 -->|"是"| P3["模式 3（程式碼執行）<br/>若多租戶則沙箱化。"]
    Q3 -->|"否"| Q4{"複雜到需要多個專責代理嗎？"}
    Q4 -->|"是"| P4["模式 4（多代理編排）"]
    Q4 -->|"否"| P5["模式 1 搭配自訂工具。"]
```
