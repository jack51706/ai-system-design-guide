...
```mermaid
flowchart LR
    Client["MCP Client"] <-->|"Streamable HTTP POST /mcp<br/>(搭配 SSE 回應串流)"| Server["MCP Server"]
```
...
```mermaid
sequenceDiagram
    participant Client as Client 代理
    participant Remote as Remote 代理
    Client->>Remote: POST /tasks
    Remote-->>Client: SSE 串流（狀態更新、artifacts）
    Remote-->>Client: 任務完成（最終結果）
```
