...
flowchart LR
    subgraph Standard["標準"]
        SQK["Q, K"] --> SAM["注意力矩陣<br/>(n x n)"] --> SOUT["輸出"]
    end
    subgraph Flash["Flash"]
        FQK["Q, K"] --> FT["分塊 Tiles<br/>(block_size x block_size)"] --> FOUT["增量輸出"]
    end
```
...
```mermaid
flowchart LR
    Q["Query<br/>(上投影 Up-projected)"] --> Cache["低維度潛在快取<br/>(Low-dim Latent Cache)"]
    KV["Key, Value<br/>(下投影 Down-projected)"] --> Cache
    PM["投影矩陣<br/>(Projection Matrices)"] --> Cache
    Cache --> Out["輸出"]
```
