...
*基於 Hamel Husain 與 Shreya Shankar 的 Maven 課程，並以實作範例、可用於生產環境的程式碼，以及針對 Phoenix、Langfuse 等平台的專屬指南加以豐富*
...
- 如何運用統計校正來修正評審的誤差
- 如何閉環：把評估結果轉化為系統改進
- 如何用你選擇的可觀測性平台（Phoenix、Langfuse、Braintrust、LangSmith，或你自建的平台）完成以上所有事

**平台範例：** 本指南以 **Arize Phoenix**（開源、自架）與 **Langfuse**（開源、雲端或自架）作為主要範例。其方法論與平台無關，請依你實際使用的工具加以調整。
...
| 工具 | 類型 | 最適合 | 成本 |
|------|------|----------|------|
| **Arize Phoenix** | 開源、自架 | 功能完整、單一 Docker 容器 | 免費 |
...
**給 PM/QA：** 你不需要自己寫儀器化的程式碼。請工程師設定好追蹤，然後用網頁 UI 以視覺化方式審閱軌跡。Phoenix（`localhost:6006`）與 Langfuse（`cloud.langfuse.com` 或你自架的 URL）都提供了 UI，讓你不必寫任何程式碼就能瀏覽、搜尋並標註軌跡。
...
你可以用任何 LLM 把維度元組轉換成擬真的查詢。以下是一個與平台無關的做法，外加一個 Phoenix 專屬的批次做法：
...
1. 打開你的軌跡檢視器（Phoenix UI、Langfuse 儀表板，或任何工具）
2. 看第一筆軌跡
3. 快速掃過它：
   - 讀使用者訊息
   - 檢查 AI 是否呼叫了正確的工具
...
1. 請工程團隊設定好追蹤（Phoenix、Langfuse，或任何工具）
2. 打開軌跡檢視器 UI
...
使用你平台內建的 UI（Phoenix 與 Langfuse 都有儀表板），或用評估結果自行建立一個簡單的儀表板。
...
**人們的做法：** 直接在生產環境編輯系統提示。
**修正方式：** 使用你平台的提示管理功能（Phoenix、Langfuse 等）來為提示做版本控管。記錄每筆追蹤紀錄使用的是哪個版本。
...
Phoenix 與 Langfuse 都內建提示版本管理。請善加利用。
...
12. **良好的可觀測性工具讓工作流程快 10 倍** - 在單一平台（Phoenix、Langfuse 等）整合追蹤、評估、資料集與實驗，相較於拼湊自製腳本，能節省大量時間。
...
1. 第 1 週：設定可觀測性（Phoenix、Langfuse 或你選用的工具），進行錯誤分析
...
*本指南受 Hamel Husain 與 Shreya Shankar 的 AI Evals for Engineers & PMs 課程啟發並以其為基礎，並補充了額外研究、可用於生產環境的程式碼範例，以及涵蓋 Phoenix、Langfuse 與更廣泛評估工具生態系的多平台指南。*
