...
```mermaid
flowchart TD
    A["使用者請求"] --> B["第 1 層：輸入驗證<br/>- 速率限制<br/>- 輸入長度限制<br/>- 基本清理"]
    B --> C["第 2 層：輸入分類<br/>- 偵測注入嘗試<br/>- 分類意圖<br/>- 標記可疑模式"]
    C --> D["第 3 層：上下文安全<br/>- 以權限為基礎的檢索<br/>- 資料存取控制<br/>- 內容清理"]
    D --> E["第 4 層：LLM 生成<br/>- 安全的系統提示<br/>- 指令階層<br/>- 安全防護機制"]
    E --> F["第 5 層：輸出驗證<br/>- 內容過濾<br/>- PII 偵測<br/>- 系統提示外洩偵測"]
    F --> G["第 6 層：安全的輸出處理<br/>- 不直接執行<br/>- 參數化操作<br/>- 跳脫渲染"]
    G --> H["回應給使用者"]
```
