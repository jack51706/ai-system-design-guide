...
## 代理式系統問題 {#agentic-systems-questions}
...
```mermaid
flowchart TD
    A["接案分流<br/>(初步分類)"] --> B["檢索研究<br/>(RAG 檢索)"]
    B --> C{"能否回答？"}
    C -->|"否"| D["升級轉接"]
    C -->|"是"| E["回應"]
    E --> F["確認<br/>(使用者是否滿意？)"]
...
---
## 生產環境與 MLOps 問題 {#production-and-mlops-questions}
...
```mermaid
flowchart TD
    A["提出變更"] --> B["跑黃金測試集<br/>(回歸偵測)"]
    B --> C["跑評估套件<br/>(品質指標)"]
    C --> D["檢查品質閘門<br/>(faithfulness > 0.85 等)"]
    D --> E["金絲雀部署<br/>(5% 流量)"]
    E --> F["監控生產環境指標"]
    F --> G["全量上線或回滾"]
```
...
```mermaid
flowchart TD
    A["進入的查詢"] --> B["輕量 embedding 模型<br/>(例如 text-embedding-3-small)"]
    B --> C["對叢集中心做相似度搜尋"]
    C --> D{"路由到合適的模型層級"}
    D --> TA["Tier A（簡單）：Gemini 3.1 Flash ($0.10/1M)<br/>或 DeepSeek V4 Flash ($0.14/1M)<br/>事實型問答、抽取"]
    D --> TB["Tier B（複雜）：Claude Sonnet 4.6 ($3/1M)<br/>推理、程式碼審查"]
    D --> TC["Tier C（推理）：Claude Opus 4.8 ($5/1M)<br/>或 GPT-5.5 reasoning ($5/1M)<br/>數學、邏輯問題"]
```
...
flowchart TD
    A["請求"] --> R["智慧路由器"]
    R --> P["主要：Claude Sonnet 4.6<br/>(70% 流量)"]
    R --> S["次要：GPT-5.5<br/>(25% 流量，驗證主要)"]
    R --> F["回退：Gemini 3.1 Flash<br/>(5%，緊急)"]
    H["每 30 秒健康檢查：<br/>P95 延遲 > 5s 降低流量占比<br/>錯誤率 > 2% 故障轉移<br/>接近速率上限 預先轉移流量"] -.-> R
```
...
## 進階問題 - 2026 年 5 月 {#advanced-questions--may-2026}
...
## 進階問題 - 2026 年 6 月 {#advanced-questions--june-2026}
