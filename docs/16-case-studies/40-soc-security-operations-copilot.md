...
```mermaid
flowchart TB
    subgraph Sources["Alert Sources - 50K+/day"]
        SIEM[SIEM: Splunk ES, Sentinel, Elastic]
        EDR[EDR: CrowdStrike Falcon, Defender XDR]
        CLOUD[Cloud and Identity Logs]
    end

    SIEM --> NORM[Normalize to OCSF plus Dedup]
    EDR --> NORM
    CLOUD --> NORM
    NORM --> CORR[Correlation Engine - group by entity]

    subgraph Enrich["Enrichment"]
        CORR --> ASSET[Asset and Identity Context: CMDB plus IAM]
        CORR --> INTEL[Threat Intel: VirusTotal plus MISP]
    ...
    BULK --> VERDICT[Verdict plus MITRE technique map plus evidence cites]
    DEEP --> VERDICT
    VERDICT --> GATE[Confidence Gate - verified signals only]

    GATE -->|high-confidence FP| CLOSE[Auto-Close plus Audit Record]
    GATE -->|unsure or TP| ESC[Escalate to Tier-2 with Written Investigation]
...
4. The bulk model (Haiku 4.5 or DeepSeek V4 Flash) drafts a first-pass verdict for each incident with a calibrated confidence, a MITRE ATT&CK mapping, and citations to the raw log lines.
...
A crafted command line, filename, or email body instructs the model to disposition the incident as benign. Mitigation: all alert content is trust-tagged as data (Decision 6); the auto-close gate reads only verified structured signals and ignores the free-text narrative, so an injected verdict cannot reach the close action; every observed injection payload is added to the red-team corpus.
...
- They gate auto-close on verified structured signals, never the free-text narrative, and make "unsure" mean escalate-with-reasoning, never silently drop.
- They name correlation, collapsing many alerts into one incident, as the core cure for alert fatigue, and enrichment (asset, identity, intel, historical memory) as where the accuracy comes from.
...
- They ground every verdict in cited raw-log evidence and a validated MITRE ATT&CK mapping so a tier-2 analyst audits rather than trusts.
