...
| Dialogue writer | Gemini 3.1 Flash (Claude Sonnet 4.7 or Opus 4.8 for premium) | Write grounded two-host banter with cited spans |
...
### A worked example: a 12-page paper becomes a two-host episode

Trace one notebook end to end. A user on the paid tier uploads a single 12-page research paper, "LATE-RERANK: Late-Interaction Reranking for Open-Domain Retrieval." Its one headline result: on the [BEIR](https://arxiv.org/abs/2104.08663) benchmark, nDCG@10 rises from 0.71 (a BM25 baseline) to 0.79 with the new reranker, about an 11 percent relative gain.

1. Ingestion parses the PDF layout-aware into about 6,200 words, chunks it into 41 spans, and embeds them into a per-notebook vector store. No other source, and no model memory, is in scope.
2. The outline planner (Gemini 3.1 Flash) budgets a 10-minute episode into six beats: the problem, prior rerankers, the method, the key result, limitations, and why it matters. Beat 4 (the key result) gets a 95-second budget, roughly 240 words.
3. Because the paper is dense, this notebook routes to the premium writer (Claude Opus 4.8). For beat 4 the writer retrieves the results-section spans and drafts an exchange between Maya (asks the listener's questions) and Leo (explains). The delivery pass, reaching for energy, rewrites Leo's line to "it improved retrieval quality by 40 percent," a figure that appears nowhere in the paper.
4. The faithfulness gate decomposes beat 4 into atomic claims and runs an entailment check (NLI, FActScore-style) against the retrieved spans. Three matter: "the method is named LATE-RERANK" (supported, abstract), "evaluated on BEIR" (supported, section 5.1), and "improved retrieval quality by 40 percent" (unsupported, because the only candidate span, table 2, says 0.71 to 0.79). The fabricated statistic is caught before any audio exists.
5. The gate returns the failing claim to the writer with the real span attached. The writer regenerates the line grounded in the number: "it pushed nDCG@10 from 0.71 to 0.79, so roughly an 11 percent relative gain over the baseline." Re-checked, every claim now entails, faithfulness_pass flips to true, and the segment is released to Stage 2.
6. Normalization pins two pronunciation traps from this paper. "BEIR" must be said "beer," not "bee eye are," and "nDCG" must be spelled out, not read as a word, so the per-notebook lexicon adds SSML before the render:
...
7. The Gemini native two-speaker call renders Maya and Leo in one pass with pinned voice IDs (for example Kore and Puck), so turn-taking and cross-speaker timing come for free. A speaker-consistency check (voice-embedding similarity to each reference, held over 0.9 per turn) confirms the two voices never drift or converge.
8. Post-processing stitches the turns, normalizes to broadcast loudness (EBU R128), and trims dead air. AudioSeal embeds an inaudible watermark, a C2PA 2.1 manifest is signed asserting the writer and TTS model IDs with digitalSourceType trainedAlgorithmicMedia, and the file ships behind a two-second audible "This episode was generated by AI" pre-roll. Wall-clock: 38 seconds, all-in cost about $0.25.

The point: the "40 percent" never reached a waveform. The gate caught it as text, where the fix is one regenerated line, not a re-rendered episode.

### The script-segment record

Each turn the writer emits is a schema-validated record, not free text, so the gate can decompose and check it deterministically. Here is beat 4's segment at the instant the gate catches the fabrication, before regeneration:
...
After the writer regrounds the single failing claim on `p8:table2`, the record re-emits with the corrected text, `supported: true` on every claim, `faithfulness_pass: true`, and a score of 1.0. Only then does the segment reach Stage 2 TTS.
...
Not every failing claim is treated the same. The gate routes each atomic claim to one of three outcomes:

| Atomic claim status | Example from the worked episode | Gate action |
|---|---|---|
| Entailed by a retrieved source span | "evaluated on BEIR" (section 5.1) | keep |
| Unsupported, but a real source value exists | "40 percent" when table 2 reports 0.71 to 0.79 | regenerate, grounded in the real span |
| Contradicted by a span (writer got it backwards) | "cut latency in half" when the paper reports 20 percent | regenerate with the corrected value |
| Unsupported with no source basis (fabrication or model memory) | "already deployed at Google" (nowhere in the paper) | drop the claim |
| Non-factual banter, question, or hedged analogy | "so it is basically a smarter librarian" | keep, tagged non-factual |

Regenerate and drop are different tools: the "40 percent" line was regrounded because a real result existed to say instead, whereas a claim with no basis at all is dropped, since there is nothing true to put in its place.

### 4. Separate the grounded content from the delivery

Natural banter and factual accuracy fight each other only if you let one model do both jobs at once. We split them. A content pass produces the grounded, cited factual skeleton (what is true, per the sources). A delivery pass rewrites that skeleton into conversation: interruptions, "oh, interesting, so does that mean...", a host asking the dumb-but-useful question, an analogy to make a dense point land. The delivery pass is explicitly constrained to **rephrase and react, never to add new facts**, and its output goes back through the faithfulness gate so an analogy that smuggles in a false claim gets caught. This is how you get "wait, 40 percent, that is huge" energy without letting the model invent the 40 percent, exactly the failure the worked example walks through. Analogies are the sharpest risk: a good one clarifies, a wrong one asserts something the source never said.
...
A confident voice mispronouncing the one term the whole episode is about ("the *SEE-quel* database" for SQL, "the *nuclear* option" mangled, a researcher's name butchered) instantly signals "a machine made this and does not understand it," and it undermines the faithfulness the rest of the pipeline worked for. We build a per-notebook pronunciation lexicon from the sources: proper nouns, product names, acronyms, and domain terms get IPA or `<phoneme>` entries. Numbers, dates, currency, and units are marked with `<say-as>` so "$1.5M" is read "one point five million dollars," not "dollar one point five em." Acronyms are classified per term (spell out "EU," say "NASA" as a word). We track a **pronunciation-error rate** on a sampled set with a lightweight ASR-round-trip check (synthesize, transcribe, compare to expected), and hard-fail an episode if a key term is mangled. In the worked episode, two terms drove lexicon entries, BEIR (pinned to the IPA for "beer" so it is not spelled out) and nDCG (forced to characters so it is not read as a word), and the ASR-round-trip check cleared both at over 0.98 string similarity before release.

### 8. Voice cloning, consent, and provenance

If the product offers custom or cloned voices, consent comes before the feature. A cloned voice requires verified opt-in from the voice owner (a spoken consent phrase captured and matched), and we never allow cloning a public figure or an uploaded sample of a third party. Every generated file carries two provenance layers, the same defense-in-depth as the [image and video pipeline](24-multimodal-generation-pipeline.md): an inaudible watermark embedded in the audio ([AudioSeal](https://github.com/facebookresearch/audioseal), [SynthID](https://deepmind.google/technologies/synthid/)) that survives recompression and clipping, and a signed C2PA manifest ([C2PA 2.1](https://c2pa.org/specifications/specifications/2.1/index.html)) declaring the model and an "AI-generated" assertion. Concretely, that manifest carries a `c2pa.actions` assertion with `digitalSourceType` set to `trainedAlgorithmicMedia` and records the writer and TTS model IDs, and the AudioSeal watermark is built to survive MP3 recompression and clipping so even a 10-second reshared cut still detects as ours (the worked episode ships both, plus the audible AI-generated pre-roll). This satisfies EU AI Act Article 50 disclosure ([Art. 50](https://artificialintelligenceact.eu/article/50/)) and gives us a detector for our own audio if a cloned voice is ever misused. The governance posture is detailed in [AI governance and compliance](../13-reliability-and-safety/04-ai-governance-and-compliance.md).
...
The loop above is the outer, beat-level view. Inside each check, every atomic claim takes the three-way route (keep, regenerate, or drop) that Decision 3's table encodes:

```mermaid
flowchart TD
    C[Atomic claim from a script segment] --> S{Entailed by a retrieved span?}
    S -->|yes| KEEP[Keep the claim]
    S -->|no| R{A real source value exists to correct it?}
    R -->|yes| REGEN[Regenerate grounded in that span]
    R -->|no| N{Asserts a checkable fact?}
    N -->|no| KEEPTAG[Keep, tagged non-factual]
    N -->|yes| DROP[Drop the claim]
    REGEN --> RECHECK[Re-decompose and re-check the segment]
    RECHECK --> S
```
...
### F9: The faithfulness gate itself waves through a subtle false claim

The gate is an entailment model, and it can pass a claim that is *almost* supported: a number rounded the wrong way, or a hedge dropped so the source's "suggests" becomes the script's "shows." Mitigation: we continuously inject **fabrication canaries**, script segments seeded with a known false statistic like the worked example's "40 percent," and alert if the gate's catch rate on them drops below target, the same canary discipline the [SOC triage copilot](40-soc-security-operations-copilot.md) uses to measure missed true-positives; the entailment threshold is calibrated on a human-labeled claim set, and approved episodes are sample-audited so a systematic miss surfaces before it becomes a trend.
...
| Fabrication-canary catch rate (injected false claims) | over 99 percent |
