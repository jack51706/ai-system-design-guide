...
```mermaid
flowchart TD
    LB["Load Balancer"] --> GPT4["GPT-4 Pool<br/>(API calls)"]
    LB --> Claude["Claude Pool<br/>(API calls)"]
    LB --> Llama["Llama 70B Pool<br/>(self-hosted)"]
```
...
```mermaid
flowchart LR
    Producers["Producers"] --> Queue["Queue<br/>(Redis / SQS)"]
    Queue --> Consumers["Consumers<br/>(LLM Workers)"]
    Consumers --> Results["Results Store"]
```
