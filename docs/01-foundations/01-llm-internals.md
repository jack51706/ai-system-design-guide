...
```mermaid
flowchart TD
    subgraph DB["Decoder Block (xN)"]
        A["Masked Self-Attention<br/>each token attends only to previous tokens"]
        B["Add and Norm"]
        C["Feed-Forward Network"]
        D["Add and Norm"]
        A --> B --> C --> D
    end
    D --> OUT["Output Probabilities"]
```
...
```mermaid
flowchart TD
    subgraph ML["MoE Layer (Decoder)"]
        ATT["Attention Layer"] --> R{"Router"}
        R --> E1["Expert 1"]
        R --> E2["Expert 2"]
        R --> E3["Expert 3"]
        R --> EN["Expert N"]
